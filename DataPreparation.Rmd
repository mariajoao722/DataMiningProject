...
loan_dev <- read_delim("Rdata/kagel/loan_comp.csv", delim = ";", 
    escape_double = FALSE, trim_ws = TRUE)

disp <- read_delim("disp.csv", delim = ";", 
    escape_double = FALSE, trim_ws = TRUE)

trans_dev <- read_delim("Rdata/kagel/trans_comp.csv", delim = ";", 
    escape_double = FALSE, trim_ws = TRUE)

account <- read_delim("account.csv", delim = ";", 
    escape_double = FALSE, trim_ws = TRUE)

card_dev <- read_delim("Rdata/kagel/card_comp.csv", delim = ";", 
    escape_double = FALSE, trim_ws = TRUE)
...
client$district_id <- as.integer(client$district_id)
...
loan_dev$status <- as.numeric(loan_dev$status)
...
```{r}
disp_temp2 <- disp_transform %>%
  filter(`type: OWNER` == "1") %>%
  select(-`type: DISPONENT`, -`type: OWNER`)

age_loan <- loan_dev_transform %>%
  left_join(disp_temp2, 
            by = "account_id",
            ) %>%
...
rm(disp_temp2, age_loan)
````




Juntar tudo o que foi considerado importante para a analise num dataset

````{r}

district_average_salary <- district_transform %>%
  select(code, `average salary`) %>%
  mutate(district_id = code) %>%
  select(-code)

disp_temp <- disp_transform %>%
  filter(`type: OWNER` == "1") %>%
  select(-`type: DISPONENT`, -`type: OWNER`)

merge_data <- loan_dev_transform %>%
  left_join(disp_temp, 
            by = "account_id",
            ) %>%
   left_join(client_transform, 
  ...
  select(
    client_id, district_id, have_card, gender_F, birth_day,
    loan_id, account_id, date, amount, duration, payments, money_in_account, age
  ) %>%
  mutate(district_id = as.integer(district_id)) %>%
  left_join(district_average_salary,
            by="district_id")
...
datasets <- list(account_transform, card_dev_transform, client_transform, disp_transform, district_name, district_transform, loan_dev_transform, trans_dev_operation, trans_dev_transform, merge_data) 

names <- list("account_transform", "card_dev_transform", "client_transform", "disp_transform", "district_name_transform", "district_transform", "loan_dev_transform", "trans_dev_operation_transform", "trans_dev_transform", "merge_data")
...
````{r}
summary(loan_dev_transform_train[, features_loan])
...
merge_data_train[, features_merge] <- scale(merge_data_train[, features_merge])


mean_values <- colMeans(merge_data_train[, features_merge], na.rm = TRUE)
standerd_deviation_values <- apply(merge_data_train[, features_merge], 2, sd, na.rm = TRUE)

merge_data_test[, features_merge] <- scale(merge_data_test[, features_merge], center = mean_values, scale = standerd_deviation_values)

rm(features_merge, mean_values, standerd_deviation_values)
````
...
save(merge_data, file="Rdata/merge_data.Rdata")
...
save(merge_data_train, file="Rdata/train/merge_data_train.Rdata")
...
save(merge_data_test, file="Rdata/test/merge_data_test.Rdata")
